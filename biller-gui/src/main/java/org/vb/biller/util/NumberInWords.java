...
public class NumberInWords {
	...
	private NumberInWords(long num) {
		this.num = num;
	}
	...
	public static NumberInWords getInstance(long num) {
		return new NumberInWords(num);
	}
	...
	private String belowLakh(long x) {
		...
		if (length(x) == 4)
			return belowTen(leftChars(x, 1)) + "Thousand "
					+ belowThousand(rightChars(x, 3));
		else
			return belowHundred(leftChars(x, 2)) + "Thousand "
					+ belowThousand(rightChars(x, 3));
	}
	...
	public String belowCrore(long x) {
		if (x < 10)
			...
			return belowLakh(x);
		if (length(x) == 6)
			return belowTen(leftChars(x, 1)) + "Lakh "
					+ belowLakh(rightChars(x, 5));
		else
			return belowHundred(leftChars(x, 2)) + "Lakh "
					+ belowLakh(rightChars(x, 5));
	}
	...
	public String belowBilion(long x) {
		...
		if (length(x) == 8)
			return belowTen(leftChars(x, 1)) + "Bilion "
					+ belowCrore(rightChars(x, 7));
		else
			return belowHundred(leftChars(x, 2)) + "Bilion "
					+ belowCrore(rightChars(x, 7));
	}
	...
	public static void main(String[] args) throws Exception {
		NumberInWords numberInWords = NumberInWords.getInstance(123456789);
		System.out.println(numberInWords.getNumberInWords());
	}
}